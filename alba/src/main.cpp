...
void printMenu(const int& userType, const bool& polarisOK, const bool& simulatorOK) {
	...
	std::cout << ""
		...
		"\t 1. Tool/NDI frames Pairing " << ((!polarisOK) ? "*" : "") << std::endl <<
...
int Menu(const int& userType,const bool& polarisOK, const bool& simulatorOK){
	...
	if (!polarisOK && (option == PAIRING || option == REGISTRATION || option == POSITIONING) ||
...
int main(int argc, const char** argv){
	...
	serialPort = (argc >= 2) ? (std::string(argv[1])) : (std::string("COM3"));
